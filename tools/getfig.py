...
if __name__ == "__main__":
    ...
    if cfg.TEST.show_rgb:
        ...
        # len_div = r_ori.shape[]
        # test_original_image1 = np.concatenate([r_ori,g_ori,b_ori],axis=-1)
        # test_original_image2 = np.concatenate([r_ori,b_ori,g_ori],axis=-1)
        # test_original_image3 = np.concatenate([g_ori,r_ori,b_ori],axis=-1)
        # test_original_image4 = np.concatenate([g_ori,b_ori,r_ori],axis=-1)
        # test_original_image5 = np.concatenate([b_ori,r_ori,g_ori],axis=-1)
    ...
    # plt.axis('off')
    # plt.imshow(test_original_image1[10])
    # plt.show()
    # plt.axis('off')
    # plt.imshow(test_original_image2[10])
    # plt.show()
    # plt.axis('off')
    # plt.imshow(test_original_image3[10])
    # plt.show()
    # plt.axis('off')
    # plt.imshow(test_original_image4[20])
    # plt.show()
    # # plt.axis('off')
    # # plt.imshow(test_original_image5[10])
    # # plt.show()
    # plt.axis('off')
    # plt.imshow(test_original_image6[20])
    # plt.show()
    ###################### Neural Network Data
    ## Training data
    # Speckle patterns
    ...
    divnum = 3
    testcls = cfg.TEST.testclass if not cfg.TEST.show_rgb else RGB_TYPE
    ori_image = y_test[0]
    rec_image = pred_test[0]
    spc_image = show_spc_image[0]
    plt.axis('off')
    plt.imshow(ori_image)
    plt.savefig(f'finalresfig/{testcls}1.tif',bbox_inches='tight',pad_inches = 0)
    plt.show()
    plt.imshow(rec_image)
    plt.axis('off')
    plt.savefig(f'finalresfig/re{testcls}1.tif',bbox_inches='tight',pad_inches = 0)
    plt.show()
    # plt.imshow(spc_image)
    # plt.axis('off')
    # plt.savefig(f'finalresfig/spc{testcls}1.tif',bbox_inches='tight',pad_inches = 0)
    # plt.show()
    ...
    #         plt.imshow(show_spc_image[i//divnum+5])
