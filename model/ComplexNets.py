...
def polar_to_rect(X): # transfer polar coordinates to rect coordinates  
...
# ParaSigmoid
class Parasigmoid(Layer):

    def __init__(self, kernel_initializer='ones',
                 kernel_regularizer=None,
                 **kwargs):
        super(Parasigmoid, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        self.beta = self.add_weight(name='beta',
                                      ...
                                      regularizer=self.kernel_regularizer,
                                      trainable=True)
        super(Parasigmoid, self).build(input_shape)
...
class Amplitude(Layer):
    ...
    def call(self, X):
        complex_X = channels_to_complex(X)
        # if self.square:
        # output = tf.square(tf.abs(complex_X))
        # else:
...
class SqrtLayer(Layer):
    ...
    def call(self, x):
        output = tf.sqrt(x)
        output = real_to_channels(output)
        return output
